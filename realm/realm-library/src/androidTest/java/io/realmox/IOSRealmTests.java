...
import io.realmox.entities.IOSAllTypes;
import io.realmox.entities.IOSChild;
...
public class IOSRealmTests {
    ...
    public void iOSDataTypes() throws IOException {
        for (String iosVersion : IOS_VERSIONS) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                ExtraTests.assertArrayEquals(new byte[]{1, 2, 3}, obj.getByteCol());
    ...
    public void iOSDataTypesDefaultValues() throws IOException {
        for (String iosVersion : IOS_VERSIONS) {
            ...
            ExtraTests.assertArrayEquals(new byte[0], obj.getByteCol());
            assertEquals("", obj.getStringCol());
            assertEquals(new Date(0), obj.getDateCol());
            assertNull(obj.getChild());
            Assert.assertEquals(0, obj.getChildren().size());
        }
    }
    ...
    public void iOSDataTypesNullValues() throws IOException {
        for (String iosVersion : IOS_VERSIONS) {
            ...
            Assert.assertEquals(new RealmList<IOSChild>(), obj.getChildren());
        }
    }
